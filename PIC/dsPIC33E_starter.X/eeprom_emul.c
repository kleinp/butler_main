...
/*******************************************************************************
 * The following functions attempt to make the EEPROM emulation easier to use
 * by having functions for writing 8, 16, and 32-bit numbers. The original
 * EEPROM emulation had addresses for every 16-bits, but since an 8-bit write is
 * introduced, there will now be an address for every 8-bit number.
 * ****************************************************************************/
unsigned char DataEERead8bit(unsigned int addr)
{
   if (addr%2 == 0)
      return((unsigned char)(DataEERead(addr/2) >> 8));
   else
      return((unsigned char)(DataEERead(addr/2) & 0xFF));
}

unsigned char DataEEWrite8bit(unsigned char data, unsigned int addr)
{
   unsigned int ee_val = DataEERead(addr/2);
   
   if (addr%2 == 0)
   {
      ee_val&=0x00FF;
      ee_val|=(unsigned int)(data) << 8;
      return(DataEEWrite(ee_val, addr/2));
   }
   else
   {
      ee_val&=0xFF00;
      ee_val|=(unsigned int)(data);
      return(DataEEWrite(ee_val, addr/2));
   }
...
unsigned int DataEERead16bit(unsigned int addr)
{
   if (addr%2 == 0)
      return(DataEERead(addr/2));
   else
      return(DataEERead8bit(addr) | DataEERead8bit(addr+1));
}

unsigned char DataEEWrite16bit(unsigned int data, unsigned int addr)
{
   unsigned char rtn;
   if (addr%2 == 0)
      return(DataEEWrite(data, addr/2));
   else
   {
      rtn = DataEEWrite8bit((unsigned char)(data >> 8), addr);
      rtn += DataEEWrite8bit((unsigned char)(data & 0xFF), addr+1);
   }
   return (rtn);
}

unsigned long DataEERead32bit(unsigned int addr)
{
   return(0);
}

unsigned char DataEEWrite32bit(unsigned long data, unsigned int addr)
{
   return(0);
}

unsigned char DataEEReadString(unsigned int len, unsigned int addr)
{
   return(0);
}

unsigned char DataEEWriteString(unsigned char *data, unsigned int len, unsigned int addr)
{
   unsigned int local_addr = addr;
   unsigned int str_len = len;
   unsigned char rtn = 0;

   // The function was called with zero length string
   if (str_len == 0)
      return(rtn);

   // If first byte is on odd 16-bit boundary, write 8-bit first
   if (addr%2 == 1)
   {
      rtn = DataEEWrite8bit(*data++, local_addr);
      local_addr++;
      str_len--;
   }
   // Write 16-bit chunks as long as possible
   while(str_len > 2)
   {
      rtn += DataEEWrite(*data++ | *data++, local_addr/2);
      local_addr+=2;
      str_len-=2;
   }
   // If we have a single byte remaining, write it
   if (str_len)
   {
      rtn += DataEEWrite8bit(*data, local_addr);
   }
